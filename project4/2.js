...
window.addEventListener("keypress", (e) => {
  ...
  // 防止form內部的button自動交出表單
  let allButtons = document.querySelectorAll("button");
  
  allButtons.forEach(button => {
    // console.log(button) // 此時會在主控台顯示所有button標籤
  ...
  allSelects.forEach((select) => {
    select.addEventListener("change", (e) => {
      // console.log(e)  // 此時選擇任一個A~F都會在主控台顯示該事件
      // console.log(e.target.value) // 選A輸出A，選B-輸出B-
      setGPA();
  ...
  function convertor(grade) {
    switch (grade) {  // 與 if statement意思相似
      case "A":
        return 4.0;
      case "A-":
        return 3.7;
  ...
  function setGPA() {
    ...
    console.log(credits.length) // credits.length = 3
    ...
    // document.getElementById("result-gpa") return element object
  ...
  function handleSorting(direction) {
    ...
    for (let i = 0; i < graders.length; i++) {
      ...
      if (
        !(  // 只要有任一項的值就會是false 就會製作class_object
          class_name == "" &&
          class_number == "" &&
          class_credit == "" &&
          class_grade == ""
    ...
    // 取得object array後，五們可以把成績String換成數字
  ...
  function merge(a1, a2) {
    ...
    while (i < a1.length && j < a2.length) {
      if (a1[i].class_grade_number > a2[i].class_grade_number) {
        result.push(a2[j]);
        j++;
      } else if (a1[i].class_grade_number < a2[i].class_grade_number) {
        result.push(a1[i]);
        i++;
      }
    }
